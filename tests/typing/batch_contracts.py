...
def check_batch_contracts(
    ...
    assert_type(collate([item]), TrainingBatch)
    ...
    masked_loader = make_target_loader(masked_dataset(store, index, mask=mask))
    assert_type(masked_loader, BatchLoader[TrainingBatch])
