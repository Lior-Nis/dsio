...
def digit_recognizer_flow(data_dir: str, workspace: str, *, seed: int = 19) -> dict[str, Any]:
    with experiment("dsio-kaggle-digit-recognizer") as parent:
        parent_id = parent.info.run_id
        data = ingest(data_dir, workspace, parent_id)
        split = split_data(data, parent_id, seed)
        pretraining = pretrain_encoder(data, split, parent_id, seed)
        training = train_classifier(data, split, pretraining, parent_id, seed)
