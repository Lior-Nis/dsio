"""Project-owned Prefect DAG for Kaggle Store Sales forecasting."""
...
from reference_projects.kaggle.store_sales.tasks import (
    SPLIT_PARAMETERS,
    TRAINING_FOLD,
    evaluate_model,
    export,
...
@flow(name="kaggle-store-sales-consumer", persist_result=False)
def store_sales_flow(data_dir: str, workspace: str, *, seed: int = 19) -> dict[str, Any]:
    experiment_id = resolve_experiment("dsio-kaggle-store-sales").experiment_id
    ...
    return {
        ...
        "fold_assignments": split["fold_assignments"],
        "fold_boundaries": split["fold_boundaries"],
        "training_fold": TRAINING_FOLD,
        "model_uri": exported["model_uri"],
        "checkpoint_digest": exported["checkpoint_digest"],
        "metrics": evaluation["metrics"],
        "prediction": inference["prediction"],
        "forecast_shape": inference["forecast_shape"],
